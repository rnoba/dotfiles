...
: "${STATE_FILE:=/root/.void-install-state}"
...
check_network() {
  ...
  if ping -c 1 -W 5 voidlinux.org &>/dev/null; then
    log_info "Network connectivity verified (ping)"
    return 0
  fi

  log_warn "ICMP ping failed, retrying over HTTPS (some networks block ping)..."

  if curl -fsS --max-time 5 -o /dev/null https://voidlinux.org; then
    log_info "Network connectivity verified (https)"
    return 0
  fi
...
install_packages() {
  ...
  xbps-install -y -r /mnt -R "$VOID_REPO" "${pkgs[@]}" || {
...
disable_service() {
  ...
  if [[ -L /mnt/etc/runit/runsvdir/default/"$svc" ]]; then
    log_debug "Disabling service: $svc"
    rm -f /mnt/etc/runit/runsvdir/default/"$svc"
  fi
}
...
state_is_done() {
  ...
  [[ -f "$STATE_FILE" ]] && grep -qxF "${module}:${phase}" "$STATE_FILE"
}

state_mark_done() {
  local module="$1" phase="$2"
  mkdir -p "$(dirname "$STATE_FILE")"
  state_is_done "$module" "$phase" || echo "${module}:${phase}" >> "$STATE_FILE"
}
