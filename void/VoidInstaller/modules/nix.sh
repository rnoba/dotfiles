...
MODULE_NAME="nix"
MODULE_DESCRIPTION="Configure Nix package manager"
MODULE_DEPENDS=("base")

readonly NIX_PACKAGES=(
	nix
)
...
# This is needed because void nix package uses an outdated version of this script that 
# does not respext XDG directory spec with use-xdg-base-directories option enabled, so we overwrite it
...
EOF
	...
	log_info "Nix configured"
}

nix_enable_service() {
	enable_service "nix-daemon"

	log_info "Nix daemon enabled"
}
